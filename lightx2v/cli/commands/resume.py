...
def add_parser(subparsers: argparse._SubParsersAction) -> None:
    parser = subparsers.add_parser("resume", help="Resume a failed or cancelled task")
...
def handle(args: argparse.Namespace) -> int:
    def _run(client, args, console):
        data = client.resume(args.task_id)
        if args.json:
            print(json.dumps(data, ensure_ascii=False, indent=2))
        else:
            console.println(data.get("msg", "ok"))
            billing = data.get("billing_info") or {}
            if billing.get("quoted_credits") is not None:
                console.println(f"quoted_credits: {billing['quoted_credits']}")
        return 0
